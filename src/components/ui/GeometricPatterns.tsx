...
export function FloatingShapes({ className }: { className?: string }) {
  return (
    <div className={cn('absolute inset-0 overflow-hidden pointer-events-none', className)}>
      ...
      <div className="absolute -top-20 -right-20 rtl:right-auto rtl:-left-20 opacity-10">
        <EightPointStar size={300} strokeWidth={1.5} animated />
      </div>

      {/* Medium star */}
      <div className="absolute top-1/3 -left-16 rtl:left-auto rtl:-right-16 opacity-10">
        <EightPointStar size={200} strokeWidth={1.5} animated />
      </div>

      {/* Small accent diamonds */}
      <div className="absolute bottom-1/4 right-1/4 rtl:right-auto rtl:left-1/4 w-10 h-10 rotate-45 border-2 border-regal-gold/15" />
      <div className="absolute top-1/4 left-1/3 rtl:left-auto rtl:right-1/3 w-6 h-6 rotate-45 bg-regal-gold/10" />
    </div>
  );
}
